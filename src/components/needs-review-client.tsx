...
function StatusBadge({ submission }: { submission: EnrichedSubmission }) {
  ...
  const variant: VariantType = (statusVariants[
    status as keyof typeof statusVariants
  ] || "default") as VariantType;
...
export function NeedsReviewClient({ submissions }: NeedsReviewClientProps) {
  ...
  const sections = selectedSubmission?.assignment?.["Test Type"]
    ? TEST_CONFIG[selectedSubmission.assignment["Test Type"]]?.sections
    : [];
  ...
  const handleOpenDialog = (submission: EnrichedSubmission) => {
    ...
    const scoreSections = submission.assignment?.["Test Type"]
      ? TEST_CONFIG[submission.assignment["Test Type"]]?.sections
      : [];

    // Check if there are existing scores to populate the form
    const existingScoresMap = new Map(
      submission.scores?.map((s) => [s.section, s.score]),
    );
  ...
  async function onScoreSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      ...
      toast({
        title: "Scores Saved",
        description: "The test scores have been recorded.",
      });
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className={cn(tableHeadStyle, "text-right")}>
                  Actions
                </TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {submissions.map((submission) => (
                <TableRow key={submission.id}>
                  ...
                  <TableCell>
                    {submission.assignment?.["Full Assignment Name"] ||
                      "Unknown Assignment"}
                  </TableCell>
                  <TableCell>
                      ...
                      <DropdownMenuContent align="end">
                        {submission.assignment?.isPracticeTest ? (
                          <>
                            ...
                            <DropdownMenuItem
                              onClick={() =>
                                handleStatusChange(submission.id, "Incomplete")
                              }
                            >
                              Mark as Incomplete
                            </DropdownMenuItem>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              Enter Scores for{" "}
              {selectedSubmission?.assignment?.["Full Assignment Name"]}
            </DialogTitle>
            <DialogDescription>
              Enter the scores for each section for{" "}
              {selectedSubmission?.student?.name}.
            </DialogDescription>
          </DialogHeader>
          <Form {...form}>
            <form
              onSubmit={form.handleSubmit(onScoreSubmit)}
