...
export function AddOfficialScoreDialog({
  ...
  const filteredPracticeTests = useMemo(() => {
    ...
    return assignments.filter(
      (a) => a.isPracticeTest && a["Test Type"] === selectedTestType,
    );
  ...
  const isStanineTest = useMemo(
    () => selectedTestType?.includes("ISEE"),
    [selectedTestType],
  );
  ...
  useEffect(() => {
    if (selectedTestType && studentId) {
      // Only run if we have both
      const config = TEST_CONFIG[selectedTestType];
      if (config) {
        const newScores = config.sections.map((s: any) => ({
          section: s.name,
          score: s.default,
        }));
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    if (!values.testType) {
      toast({
        title: "Error",
        description: "Could not determine test type for student.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      const assignmentId = isOfficial
        ? values.officialTestName
        : values.practiceTestId;
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[600px] max-h-[85vh] md:max-h-[90vh] p-0">
                              ...
                              <FormItem>
                                ...
                                >
                                  ...
                                  <SelectContent>
                                    ...
                                    <SelectItem
                                      value={`Official ${selectedTestType}`}
                                    >{`Official ${selectedTestType}`}</SelectItem>
                                    {selectedTestType === "SAT" && (
                                      <>
                                        <SelectItem value="Official PSAT/NMSQT">
                                          Official PSAT/NMSQT
                                        </SelectItem>
                                        <SelectItem value="Official PSAT 10">
                                          Official PSAT 10
                                        </SelectItem>
                                        <SelectItem value="Official PSAT 8/9">
                                          Official PSAT 8/9
                                        </SelectItem>
                                      </>
                                    )}
                          ...
                          {currentTestConfig && (
                            <div className="space-y-4 pt-4">
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4 rounded-md border p-4">
                                {currentTestConfig.sections.map(
                                  (section: any, index: number) => {
                                    const currentScore = watch(
                                      `scores.${index}`,
                                    ) || {
                                      section: section.name,
                                      score: section.default,
                                    };

                                    return (
                                      <div
                                        key={`${selectedTestType}-${section.name}-${index}`}
                                      >
