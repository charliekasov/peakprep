...
// Centralized score field mapping
const SCORE_FIELD_MAPPING: Record<string, string> = {
  'Math Score': 'Math',
...
// Single source of truth for Firebase document transformation
export function fromFirebase(doc: DocumentSnapshot): Submission {
  ...
  // Transform legacy sheet-style score data to new scores array format
  if (!submission.scores || submission.scores.length === 0) {
    submission.scores = []; // Initialize empty array
    
    for (const [sheetHeader, sectionName] of Object.entries(SCORE_FIELD_MAPPING)) {
      const rawValue = (data as any)[sheetHeader];
      if (rawValue) {
...
export async function updateSubmission(
  submissionId: string, 
  updates: Partial<{ status: SubmissionStatus; scores: { section: string; score: number }[] }>
): Promise<void> {
