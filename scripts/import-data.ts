...
if (!SERVICE_ACCOUNT_PATH) {
  ...
  console.error(
    "Please point it to your service account key file in your .env file.",
  );
  process.exit(1);
}
...
async function importCollectionFromSheet(
  collectionName: string,
  sheetNameOrNames: string | string[],
) {
  ...
  console.log(
    `Found ${jsonData.length} total records to import into '${collectionName}'.`,
  );
  ...
  console.log(
    `Successfully imported ${jsonData.length} documents into '${collectionName}'.`,
  );
}
